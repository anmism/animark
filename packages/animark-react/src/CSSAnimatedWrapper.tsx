...
function getClass(type: string, dir: string): string {
  switch (type) {
    ...
    case "reveal":
      return "amk-reveal";
    case "slide":
      return `amk-slide-${dir}`;
    case "translateX":
      return dir === "right" ? "amk-translateX-right" : "amk-translateX-left";
    case "translateY":
      return dir === "down" ? "amk-translateY-down" : "amk-translateY-up";
    case "rotate":
      return dir === "left" ? "amk-rotate-ccw" : "amk-rotate-cw";
    case "rotateX":
      return "amk-rotateX";
    case "rotateY":
      return "amk-rotateY";
    ...
    case "fade-out":
      return "amk-fade-out";
    case "scale-out":
      return "amk-scale-out";
    case "blur-out":
      return "amk-blur-out";
    case "disappear":
      return "amk-disappear";
    case "reveal-out":
      return "amk-reveal-out";
    case "slide-out":
      return `amk-slide-out-${dir}`;
    case "translateX-out":
      return dir === "right"
        ? "amk-translateX-out-right"
        : "amk-translateX-out-left";
    case "translateY-out":
      return dir === "down"
        ? "amk-translateY-out-down"
        : "amk-translateY-out-up";
    case "rotate-out":
      return dir === "left" ? "amk-rotate-out-ccw" : "amk-rotate-out-cw";
    case "rotateX-out":
      return "amk-rotateX-out";
    case "rotateY-out":
      return "amk-rotateY-out";
    case "skew-out":
      return "amk-skew-out";
    case "scaleX-out":
      return "amk-scaleX-out";
    case "scaleY-out":
      return "amk-scaleY-out";
    case "pulse":
      return "amk-pulse";
    case "shake":
      return "amk-shake";
    case "wiggle":
      return "amk-wiggle";
    case "heartbeat":
      return "amk-heartbeat";
    case "float":
      return "amk-float";
    case "hover-lift":
      return "amk-hover-lift";
    case "magnetic":
      return "amk-magnetic";
    case "typing":
      return "amk-typing";
    case "highlight":
      return "amk-highlight";
    default:
      return "amk-fade";
  }
}
...
function getAmountVars(type: string, amount?: number): Record<string, string> {
  ...
  switch (type) {
    ...
    case "translateY-out":
      return {
        "--amk-amount": `${amount}px`,
        "--amk-amount-neg": `-${amount}px`,
      };
    case "rotate":
    case "rotateX":
    case "rotateY":
  ...
  function process(node: React.ReactNode): React.ReactNode {
    ...
    if (React.isValidElement<{ children?: React.ReactNode }>(node)) {
      return React.cloneElement(
        node,
        {},
        React.Children.map(node.props.children, process),
      );
    }
...
function Magnetic({
  ...
  return (
    <span
      ref={ref}
      key={playCount}
      className="amk-wrapper amk-magnetic"
...
export const CSSAnimatedWrapper: React.FC<CSSAnimatedWrapperProps> = ({
  ...
  // ── Derived ───────────────────────────────────────────────────────────────
  const isSequenced =
    trigger === "with-previous" || trigger === "after-previous";
  ...
  if (isSequenced && reg === null) {
    return (
      <span style={{ visibility: isExit ? "visible" : "hidden" }}>
        {children}
      </span>
    );
  }
  if (!isActivated) {
    // Highlight text must always be readable — only the background sweep is deferred.
    // Exit animations also start visible. Everything else hides until triggered.
    const alwaysVisible = isExit || type === "highlight";
    return (
      <span style={{ visibility: alwaysVisible ? "visible" : "hidden" }}>
        {children}
      </span>
    );
  }

  const animKey = isOnClick ? `${playCount}-click-${clickIndex}` : playCount;
  ...
  if (type === "typing") {
    return (
      <span
        key={animKey}
        className={`amk-wrapper amk-typing ${className}`.trim()}
        style={style}
      >
        <TypingInner
          baseDelay={effectiveDelay}
          charDelay={charDelay}
          playKey={animKey as number}
        >
          {children}
        </TypingInner>
      </span>
  ...
  if (type === "highlight") {
    return (
      <span
        ...
        style={
          {
            "--amk-hl-duration": `${duration}ms`,
            "--amk-hl-delay": `${effectiveDelay}ms`,
            ...style,
  ...
  if (type === "hover-lift") {
    return (
      <span className={`amk-hover-lift ${className}`.trim()} style={style}>
        {children}
      </span>
    );
  }
  ...
  const inlineStyle: React.CSSProperties = {
    ...
    ...(Object.keys(amountVars).length > 0 &&
      (amountVars as React.CSSProperties)),
    ...style,
  } as React.CSSProperties;
